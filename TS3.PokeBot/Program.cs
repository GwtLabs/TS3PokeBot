...
namespace TS3.PokeBot
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            IQueryClient client = new QueryClient(notificationHub: notifications, keepAliveInterval: TimeSpan.FromSeconds(30), host: settings.Server.Host, port: settings.Server.QueryPort);
            ...
            Console.WriteLine("Admin login:" + !new LoginCommand(settings.Server.Login, settings.Server.Password).Execute(client).IsErroneous);
            Console.WriteLine("Switch to server with port 9987: " + !new UseCommand(settings.Server.Port).Execute(client).IsErroneous);
